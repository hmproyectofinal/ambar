...
MAGESTORE_NAMESPACE_MANAGER.registerNamespace('magestore.storepickup');
...
//			console.trace();
	...
	updateStoreList: function() {
		...
		for (var i = 0; i < markers.length; i++) {
			var storeid = markers[i].options.store_id;

			if (storeid != '') {
				jQuery('#store-item-'+storeid).show();
			}
		}
	},
	...
	clickMarkerCallBack: function(event, store) {
		// console.info(event);
		// console.info(store);
		store.store_view_url = '/storepickup/'+store.url_id_path+'.html';
		this.openInfoWindow(store);
		if(!jQuery('.store-results').is(':visible')){
			jQuery('.store-results').slideToggle();
		}
        ...
        jQuery('#store-item-'+store.store_id).addClass('active');
        
        var offset = jQuery('.store-results .store-item').first().position().top;
        
       if(jQuery(window).width() < 768){
   	        jQuery('.store-results').animate({
   	            scrollTop: (jQuery('#store-item-'+store.store_id).offset().top - offset - 330)
   	        },100);
   	    }else{
   	    	jQuery('.store-results').animate({
   	    	    scrollTop: (jQuery('#store-item-'+store.store_id).offset().top - offset - 590)
   	    	},100);
   	    }
	},
	...
	initListStore: function() {
		...
		$A(this.options.listStoreJson).each((function(storeJson) {
			...
			marker = store.getMarker();
			marker.options = storeJson;
	...
	renderListStoreToHtml: function(listStore) {
		// if ($(this.options.storeOptions.listStoreBoxId)) {
			...
			$A(listStore).each((function(store) {
				var html  = '<div id="store-item-'+store.store_id+'" class="widget-runway-subview-card-view-container store-item store">';
				    html +=     '<div class="contenedorInfo">';
				    html +=         '<div class="name">Nombre: '+store.store_name+'</div>';
				    html +=         '<div class="telephone">Teléfono: '+store.store_phone+'</div>';
				    html +=         '<div class="address">Direccíon: '+store.address+'</div>';
				    html +=         '<div class="city">Ciudad: '+store.city+'</div>';
				    html +=         '<div class="region">Provincia: '+store.state+'</div>';
				    html +=         '<div class="country">País: '+store.country_name+'</div>';
				    if (store.pickup_enabled==1) {
				        html +=     '<div class="pickup_enable">'+store.pickup_enabled+'</div>'
				    }
				    html += 	'</div>';
				    html += '</div>';
				    
				jQuery(html).appendTo('#storelocator .store-results');
...
var fadeEffect = function() {
	return {
		init: function(id, flag, target) {
			...
			if (jQuery(this.elem).length > 0){//Fix Validacion - error de consola.
