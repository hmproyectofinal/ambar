...
class Magestore_Storepickup_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function exportAllStoreMultiViewToJson()
    {
        ...
        foreach ($storeviews as $key => $storeview) {
            ...
            $this->exportAllStoreToJson($storeview->getId(),true);
        }
        //Export for store view default
        $this->exportAllStoreToJson();
        $this->exportAllStoreToJson(null, true);
    }
    public function exportAllStoreToJson($storeViewId = null, $pickupfile = false)
    {

        if (isset($storeViewId)) {
            $filePath = Mage::getBaseDir('media') . '/storepickup/liststore' . ($pickupfile ? 'pickup' : '')  . '_storeview' . $storeViewId . '.js';
        } else {
            $filePath = Mage::getBaseDir('media') . '/storepickup/liststore' . ($pickupfile ? 'pickup' : '')  .'.js';
        }
        ...
        if ($pickupfile) $storeCollection->addFieldToFilter('pickup_enabled', 1);
