...
class Magestore_Storepickup_Model_Observer
{
    ...
    public function onestepcheckout_index_save_shipping($observer)
    {
        ...
        Mage::log('onestepcheckout_index_save_shipping');
    ...
    public function save_storeaddress($order)
    {
        ...
        try {
            ...
            if ($store) {
                ...
                /* FIX UPDATE ORDER SHIPPING AFTER ORDER PLACE */
                $orderShippingAddress = Mage::getModel('sales/order_address')->load($order->getShippingAddress()->getId());

                $orderShippingAddress
                ->setFirstname(Mage::helper('storepickup')->__('Store'))
                ->setLastname($store->getStoreName())
                ->setStreet($store->getAddress())
                ->setCity($store->getCity())
                ->setCountry_id($store->getCountry())
                ->setRegion($store->getState())
                ->setRegion_id($store->getStateId())
                ->setPostcode($store->getZipcode())
                ->setFax($store->getFax())->save();
            }
